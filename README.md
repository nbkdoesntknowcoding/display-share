...
| **Settings shows Display Share enabled, but the app still asks for permission** | Two separate causes, both about identity. macOS ties permissions to the code signature, and an unsigned app is identified by the **hash of its binary** — so *every rebuild is a new app* and the old grant no longer applies. A stale entry from the previous copy still shows in the list, looking identical | Remove the entry with **–**, grant again, then **stop reinstalling**. Rebuilding in a loop to fix a permission problem is what keeps it broken |
